...
contains
  ...
  subroutine FatesPlantRespPhotosynthDrive (nsites, sites,bc_in,bc_out,dtime)
    ...
    real(r8) :: gs_av_z(nlevleaf,maxpft,nclmax) 
               ...
               if(currentPatch%countcohorts > 0.0)then   ! Ignore empty patches
                  ...
                  do while (associated(currentCohort)) ! Cohort loop
                     ...
                     if(currentPatch%canopy_mask(cl,ft) == 1)then 
                        ...
                        currentCohort%stomatal_cond = 0.0_r8
                        ...
                        currentCohort%stomatal_cond(1:nv) = gs_av_z(1:nv,ft,cl)
                     ...
                     currentCohort%stomatal_cond = currentCohort%stomatal_cond *dtime
  ...
  subroutine LeafLayerPhotosynthesis(f_sun_lsl,         &  ! in
   ...
   real(r8), intent(out) :: gs_av_out    ! stomatal conductance (umolH2O/m**2/s)
   ...
   associate( bb_slope  => EDPftvarcon_inst%BB_slope      ,& ! slope of BB relationship, unitless
     ...
     if ( parsun_lsl <= 0._r8 ) then  ! night time
        ...
        gs_av_out = stomatal_intercept_btran       
         ...
         if ( laisun_lsl + laisha_lsl > 0._r8 ) then 
           ...
           do  sunsha = 1,2      
              ...
              if(sunsha == 1)then !sunlit       
                 ...
                 gs_av_out = gs_av_out + (max(gs_mol,cf/rsmax0)) *f_sun_lsl
              else
                 ...
                 gs_av_out = gs_av_out + (max(gs_mol,cf/rsmax0)) *(1.0_r8-f_sun_lsl) 
              end if
  ...
  subroutine quadratic_f (a, b, c, r1, r2)
   ...
   subroutine LeafLayerBiophysicalRates( parsun_lsl, &
      ...
      real(r8), intent(in) :: veg_tempk           ! vegetation temperature
      real(r8), intent(in) :: t10             !10-day running mean of 2-m temperature (Kelvin)
      real(r8), intent(in) :: btran           ! transpiration wetness factor (0 to 1) 
      ...
      real(r8) :: jvr            ! ratio of jmax:vcmax
      ...
      !vcmaxha = EDPftvarcon_inst%vcmaxha(FT) !default
      vcmaxha = (42.6_r8+1.14_r8*min(max((t10-tfrz),3._r8),37._r8))*1000    !Kumarathunge, change unit from kJ/mol to J/mol
      jvr = (2.56_r8 -0.0202_r8*min(max((t10-tfrz),3._r8),37._r8))/1.67_r8     ! Kumarathunge, jamx:vcmax, 1.67 is the default
      !value set in src/fates/main/FatesParameterDerivedMod.F90
      !jvr  = (2.59_r8 - 0.035_r8*min(max((t10-tfrz),11._r8),35._r8))/1.67_r8   ! Kattage
      ...
      !vcmaxse = EDPftvarcon_inst%vcmaxse(FT) !default
      vcmaxse = 645.13_r8 - 0.38_r8 * min(max((t10-tfrz),3._r8),37._r8) !Kumarathunge
      !vcmaxse = 668.39_r8 - 1.07_r8 * min(max((t10-tfrz),11._r8),35._r8) !Kattage
      !jmaxse  = EDPftvarcon_inst%jmaxse(FT)  !default
      jmaxse  = 658.77_r8 - 0.52_r8*min(max((t10-tfrz),3._r8),37._r8)   !Kumarathunge
      !jmaxse  = 659.70_r8 - 0.75_r8 * min(max((t10-tfrz),11._r8),35._r8) !Kattage
      ...
      if ( parsun_lsl <= 0._r8) then           ! night time
         ...
         jmax25  = jmax25top_ft * nscaler * jvr
